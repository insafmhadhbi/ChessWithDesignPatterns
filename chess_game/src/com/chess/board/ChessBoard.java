...
public class ChessBoard extends JPanel implements Subject {
    ...
    private void handleSquareClick(int row, int col) {
        ...
        if (selectedSquare == null) {
            ...
            if (gameRules.isValidMove(selectedSquare, clickedSquare, squares)) {
                ...
                boolean wouldLeaveKingInCheck = gameRules.wouldMoveLeaveKingInCheck(selectedSquare, clickedSquare,
                        squares);

                if (wouldLeaveKingInCheck) {
                    // Show a warning but allow the player to continue with the move
                    int option = JOptionPane.showConfirmDialog(this,
                            "Warning: This move would leave your King in check. Do you want to proceed?",
                            "Invalid Move",
                            JOptionPane.YES_NO_OPTION,
                            JOptionPane.WARNING_MESSAGE);

                    if (option == JOptionPane.NO_OPTION) {
                        // Player decided not to proceed with the move
                        return;
                    }
                }
                ...
                // Check if the opponent's king is in checkmate or has been captured
                ...
                if (gameRules.isCheckmate(opponent, squares)) {
                    ...
                    String winner = (currentPlayer == Player.WHITE) ? "White" : "Black";
                ...
                // Check for stalemate
                if (gameRules.isStalemate(opponent, squares)) {
                    currentGameState = GameState.STALEMATE;
                    game.setGameState(currentGameState);
    ...
    private void movePiece(Square from, Square to) {
        ...
        if (to.getPiece() != null) {
            ...
            if (to.getPiece() instanceof King) {
                // Current player wins by capturing the opponent's king
                Player currentPlayer = ChessGame.getInstance().getCurrentPlayer();
                currentGameState = (currentPlayer == Player.WHITE) ? GameState.WHITE_WIN : GameState.BLACK_WIN;
                ChessGame.getInstance().setGameState(currentGameState);

                // Display victory message
                String winner = (currentPlayer == Player.WHITE) ? "White" : "Black";
                JOptionPane.showMessageDialog(this,
                        "Game Over! " + winner + " wins by capturing the opponent's king!",
                        "Game Over",
                        JOptionPane.INFORMATION_MESSAGE);
    ...
    public void resetBoard() {
        ...
        // Reinitialize the board with new pieces
        initializeBoard();
        setupPieces();
        repaint();
    }
    ...
    private void drawGameOverOverlay(Graphics2D g2d) {
        ...
        // Get current game state and player
        ChessGame game = ChessGame.getInstance();
        Player currentPlayer = game.getCurrentPlayer();

        // Check for game over conditions (checkmate or stalemate)
        if (gameRules.isCheckmate(currentPlayer, squares)) {
            message = (currentPlayer == Player.WHITE) ? "CHECKMATE!\nBlack Wins!" : "CHECKMATE!\nWhite Wins!";
        } else if (gameRules.isStalemate(currentPlayer, squares)) {
            message = "STALEMATE!\nGame Drawn";
        }

        // If no game over state, return without drawing overlay
        if (message.isEmpty())
            return;
