...
public class GameRules {
    ...
    public boolean isCheckmate(Player player, Square[][] board) {
        ...
        // Try every possible move for every piece of the player
        for (int fromRow = 0; fromRow < 8; fromRow++) {
            for (int fromCol = 0; fromCol < 8; fromCol++) {
                Square fromSquare = board[fromRow][fromCol];
                if (fromSquare.getPiece() != null && fromSquare.getPiece().getPlayer() == player) {
                    ...
                    for (int toRow = 0; toRow < 8; toRow++) {
                        for (int toCol = 0; toCol < 8; toCol++) {
                            Square toSquare = board[toRow][toCol];
                            // Check if this move is valid and would get the king out of check
                            if (isValidMove(fromSquare, toSquare, board)) {
                                // Simulate the move
                                Piece originalFromPiece = fromSquare.getPiece();
                                Piece originalToPiece = toSquare.getPiece();
                                toSquare.setPiece(fromSquare.getPiece());
                                fromSquare.setPiece(null);

                                // Check if the king is still in check after the move
                                boolean kingStillInCheck = isKingInCheck(player, board);

                                // Undo the move
                                fromSquare.setPiece(originalFromPiece);
                                toSquare.setPiece(originalToPiece);
